...
func (scs ScreenState) String() string {
	return fmt.Sprintf("uc:(%v) gridBounds:%v", scs.Cursor, scs.Grid.Bounds())
}
...
func (scs *ScreenState) Update(w io.Writer, prior ScreenState) (int, ScreenState, error) {
	n, _, err := withAnsiWriter(w, prior.Cursor, func(aw ansiWriter, cur CursorState) (n int, _ CursorState) {
		prior.Cursor = cur
...
func (scs *ScreenState) update(aw ansiWriter, prior ScreenState) (int, ScreenState) {
	...
	var n, m int
	n += aw.WriteSeq(cur.Hide())
	m, cur = writeGrid(aw, cur, scs.Grid, prior.Grid, NoopStyle)
	n += m
